...
manifest.setHostPermissions(...(process.env.HOST_PERMISSIONS || '').split(",").map(it => it.trim()));
manifest.setName(process.env.EXTENSION_NAME);
manifest.setDescription(process.env.EXTENSION_DESCRIPTION);
manifest.save();
...
const config = {
    ...
    plugins: [
        ...
        new CopyPlugin({
            patterns: [
                {from: path.join(__dirname, `./src/manifest.json`), to: path.join(__dirname, './extension/manifest.json')}
            ]
        }),
    ]
};
